...
pub(super) fn setup_device(instance: &Instance, graphics_device: &DeviceSelectionInfo, options: &GraphicsOptions) -> Option<DeviceCreationResult> {
    ...
    let mut graphics_family = graphics_device.device_queue_info
        .iter().find(|e|{
        e.is_graphics_queue()
    }).unwrap();
    ...
    if options.use_transfer_queues  {
        if let Some(family) = graphics_device.device_queue_info
            .iter().find(|e| e.is_transfer_only()) {
    ...
    let device = unsafe {
        instance.create_device(graphics_device.device, &device_create_info, None).ok()?
    };
