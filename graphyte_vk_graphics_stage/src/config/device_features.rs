...
pub(crate) fn meets_required_features(
    ...
    has: vk::PhysicalDeviceFeatures,
) -> bool {
    if required.robust_buffer_access == vk::TRUE && has.robust_buffer_access == vk::FALSE {
        return false;
    }
    if required.full_draw_index_uint32 == vk::TRUE && has.full_draw_index_uint32 == vk::FALSE {
        return false;
    }
    if required.image_cube_array == vk::TRUE && has.image_cube_array == vk::FALSE {
        return false;
    }
    if required.independent_blend == vk::TRUE && has.independent_blend == vk::FALSE {
        return false;
    }
    if required.geometry_shader == vk::TRUE && has.geometry_shader == vk::FALSE {
        return false;
    }
    if required.tessellation_shader == vk::TRUE && has.tessellation_shader == vk::FALSE {
        return false;
    }
    if required.sample_rate_shading == vk::TRUE && has.sample_rate_shading == vk::FALSE {
        return false;
    }
    if required.dual_src_blend == vk::TRUE && has.dual_src_blend == vk::FALSE {
        return false;
    }
    if required.logic_op == vk::TRUE && has.logic_op == vk::FALSE {
        return false;
    }
    if required.multi_draw_indirect == vk::TRUE && has.multi_draw_indirect == vk::FALSE {
        return false;
    }
    if required.draw_indirect_first_instance == vk::TRUE
        && has.draw_indirect_first_instance == vk::FALSE
    {
        return false;
    }
    if required.depth_clamp == vk::TRUE && has.depth_clamp == vk::FALSE {
        return false;
    }
    if required.depth_bias_clamp == vk::TRUE && has.depth_bias_clamp == vk::FALSE {
        return false;
    }
    if required.fill_mode_non_solid == vk::TRUE && has.fill_mode_non_solid == vk::FALSE {
        return false;
    }
    if required.depth_bounds == vk::TRUE && has.depth_bounds == vk::FALSE {
        return false;
    }
    if required.wide_lines == vk::TRUE && has.wide_lines == vk::FALSE {
        return false;
    }
    if required.large_points == vk::TRUE && has.large_points == vk::FALSE {
        return false;
    }
    if required.alpha_to_one == vk::TRUE && has.alpha_to_one == vk::FALSE {
        return false;
    }
    if required.multi_viewport == vk::TRUE && has.multi_viewport == vk::FALSE {
        return false;
    }
    if required.sampler_anisotropy == vk::TRUE && has.sampler_anisotropy == vk::FALSE {
        return false;
    }
    if required.texture_compression_etc2 == vk::TRUE && has.texture_compression_etc2 == vk::FALSE {
        return false;
    }
    if required.texture_compression_astc_ldr == vk::TRUE
        && has.texture_compression_astc_ldr == vk::FALSE
    {
        return false;
    }
    if required.texture_compression_bc == vk::TRUE && has.texture_compression_bc == vk::FALSE {
        return false;
    }
    if required.occlusion_query_precise == vk::TRUE && has.occlusion_query_precise == vk::FALSE {
        return false;
    }
    if required.pipeline_statistics_query == vk::TRUE && has.pipeline_statistics_query == vk::FALSE
    {
        return false;
    }
    if required.vertex_pipeline_stores_and_atomics == vk::TRUE
        && has.vertex_pipeline_stores_and_atomics == vk::FALSE
    {
        return false;
    }
    if required.fragment_stores_and_atomics == vk::TRUE
        && has.fragment_stores_and_atomics == vk::FALSE
    {
        return false;
    }
    if required.shader_tessellation_and_geometry_point_size == vk::TRUE
        && has.shader_tessellation_and_geometry_point_size == vk::FALSE
    {
        return false;
    }
    if required.shader_image_gather_extended == vk::TRUE
        && has.shader_image_gather_extended == vk::FALSE
    {
        return false;
    }
    if required.shader_storage_image_extended_formats == vk::TRUE
        && has.shader_storage_image_extended_formats == vk::FALSE
    {
        return false;
    }
    if required.shader_storage_image_multisample == vk::TRUE
        && has.shader_storage_image_multisample == vk::FALSE
    {
        return false;
    }
    if required.shader_storage_image_read_without_format == vk::TRUE
        && has.shader_storage_image_read_without_format == vk::FALSE
    {
        return false;
    }
    if required.shader_storage_image_write_without_format == vk::TRUE
        && has.shader_storage_image_write_without_format == vk::FALSE
    {
        return false;
    }
    if required.shader_uniform_buffer_array_dynamic_indexing == vk::TRUE
        && has.shader_uniform_buffer_array_dynamic_indexing == vk::FALSE
    {
        return false;
    }
    if required.shader_sampled_image_array_dynamic_indexing == vk::TRUE
        && has.shader_sampled_image_array_dynamic_indexing == vk::FALSE
    {
        return false;
    }
    if required.shader_storage_buffer_array_dynamic_indexing == vk::TRUE
        && has.shader_storage_buffer_array_dynamic_indexing == vk::FALSE
    {
        return false;
    }
    if required.shader_storage_image_array_dynamic_indexing == vk::TRUE
        && has.shader_storage_image_array_dynamic_indexing == vk::FALSE
    {
        return false;
    }
    if required.shader_clip_distance == vk::TRUE && has.shader_clip_distance == vk::FALSE {
        return false;
    }
    if required.shader_cull_distance == vk::TRUE && has.shader_cull_distance == vk::FALSE {
        return false;
    }
    if required.shader_float64 == vk::TRUE && has.shader_float64 == vk::FALSE {
        return false;
    }
    if required.shader_int64 == vk::TRUE && has.shader_int64 == vk::FALSE {
        return false;
    }
    if required.shader_int16 == vk::TRUE && has.shader_int16 == vk::FALSE {
        return false;
    }
    if required.shader_resource_residency == vk::TRUE && has.shader_resource_residency == vk::FALSE
    {
        return false;
    }
    if required.shader_resource_min_lod == vk::TRUE && has.shader_resource_min_lod == vk::FALSE {
        return false;
    }
    if required.sparse_binding == vk::TRUE && has.sparse_binding == vk::FALSE {
        return false;
    }
    if required.sparse_residency_buffer == vk::TRUE && has.sparse_residency_buffer == vk::FALSE {
        return false;
    }
    if required.sparse_residency_image2_d == vk::TRUE && has.sparse_residency_image2_d == vk::FALSE
    {
        return false;
    }
    if required.sparse_residency_image3_d == vk::TRUE && has.sparse_residency_image3_d == vk::FALSE
    {
        return false;
    }
    if required.sparse_residency2_samples == vk::TRUE && has.sparse_residency2_samples == vk::FALSE
    {
        return false;
    }
    if required.sparse_residency4_samples == vk::TRUE && has.sparse_residency4_samples == vk::FALSE
    {
        return false;
    }
    if required.sparse_residency8_samples == vk::TRUE && has.sparse_residency8_samples == vk::FALSE
    {
        return false;
    }
    if required.sparse_residency16_samples == vk::TRUE
        && has.sparse_residency16_samples == vk::FALSE
    {
        return false;
    }
    if required.sparse_residency_aliased == vk::TRUE && has.sparse_residency_aliased == vk::FALSE {
        return false;
    }
    if required.variable_multisample_rate == vk::TRUE && has.variable_multisample_rate == vk::FALSE
    {
        return false;
    }
    if required.inherited_queries == vk::TRUE && has.inherited_queries == vk::FALSE {
        return false;
    }
...
pub(crate) fn disabled_device_features() -> vk::PhysicalDeviceFeatures {
    vk::PhysicalDeviceFeatures {
